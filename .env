...
3.  **On-Chain Data Pillar (Dynamic)**: This is a key innovation introduced in v3.1 and has been optimized thoroughly since then. The system dynamically identifies the top 50 "whale" accounts by querying the `xrpscan.com` API. It then analyzes the last 12 hours of transactions for these accounts to detect significant market pressure from major players.
...
| `PREDICTION_HORIZONS`      | A comma-separated list of future hours to predict.                          | `48`         | e.g., `6,24,48`. The script trains a separate model for each horizon. - |
...
| `OPTIMIZE_HYPERPARAMETERS` | Set to `True` to run a lengthy Optuna study to find the best model params.  | `False`      | Recommend: keep `False` for daily runs. Use for periodic deep tuning. - | 
| `OPTUNA_TRIALS`            | The number of trials to run during the optimization study.                  | `50`         | Active if above is `True`. More trials = better models + take long. |
...
├── clairvoyant.py   # The core script containing all logic
